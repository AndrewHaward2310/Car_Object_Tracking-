...
void moveCar(int inputValue) {
    switch(inputValue) {
        case UP:
            printf("MOVE UP\n");
            sendToSerial("MOVE UP");
            break;
        case DOWN:
            printf("MOVE DOWN\n");
            sendToSerial("MOVE DOWN");
            break;
        case LEFT:
            printf("MOVE LEFT\n");
            sendToSerial("MOVE LEFT");
            break;
        case RIGHT:
            printf("MOVE RIGHT\n");
            sendToSerial("MOVE RIGHT");
            break;
        case STOP:
            printf("MOVE STOP\n");
            sendToSerial("MOVE STOP");
            break;
        default:
            sendToSerial("STOP");
            break;
...
// Handler for car movement API
static esp_err_t move_car_handler(httpd_req_t *req) {
    char* buf;
    size_t buf_len;
    int move_command;

    // Get the length of the request URL
    buf_len = httpd_req_get_url_query_len(req) + 1;
    if (buf_len > 1) {
        buf = (char*)malloc(buf_len);
        if (buf == NULL) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            char param[32];
            // Get the value of the `move` parameter
            if (httpd_query_key_value(buf, "move", param, sizeof(param)) == ESP_OK) {
                move_command = atoi(param);
                moveCar(move_command);
                free(buf);
                // Send the HTTP response
                return httpd_resp_send(req, "Car move command executed", HTTPD_RESP_USE_STRLEN);
            }
        }
        free(buf);
    }
...
void startCameraServer()
{
    ...
    httpd_uri_t move_car_uri = {
        .uri = "/move_car",
        .method = HTTP_GET,
        .handler = move_car_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &move_car_uri);
    }
